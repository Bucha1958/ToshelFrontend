...
import React, { useState } from 'react';
import { FaEnvelope, FaPhoneAlt, FaMapMarkerAlt, FaPaperPlane, FaBuilding } from 'react-icons/fa';
...
const SidebarContact = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <div className={`fixed top-0 right-0 h-full w-full md:w-full lg:w-5/12 bg-white z-50 text-black transition-transform transform duration-700 font-poppins ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      ...
      <div className="overflow-y-auto h-full p-6 sm:p-10 space-y-10">
        {/* Title */}
        <h2 className="text-3xl font-semibold text-gray-800 border-b pb-2">CONTACT US</h2>
        ...
        <div className="space-y-5">
          <h3 className="text-xl font-semibold text-gray-800 flex items-center gap-2">
            <FaBuilding className="text-blue-600" /> Company Info
          </h3>
          <div className="flex items-start gap-3">
            <FaMapMarkerAlt className="text-blue-600 mt-1" />
            <div>
              <p><strong>Corporate Office:</strong><br />4 Nimo Street, New Haven, Enugu</p>
              <p className="mt-2"><strong>Site Office:</strong><br />Naira Triangle, Enugu</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <FaEnvelope className="text-blue-600" />
        ...
        <div className="mt-10 bg-blue-50 rounded-lg p-4 shadow-md text-center">
          <h3 className="text-lg font-semibold text-gray-800">Subscribe to Our Newsletter</h3>
          <p className="text-sm text-gray-600 mb-4">Stay updated with our latest news and offers.</p>
          {!subscribed ? (
            <form
              className="flex flex-col sm:flex-row items-center gap-2 justify-center"
              onSubmit={handleSubscription}
